...
public class CheckPoint extends Entity {
	...
	public void draw(Graphics g, Main game) {
		if (x==game.pcStartX&&y==game.pcStartY) {
			game.flashDisplay
					.add(new Box((int) x, (int) y, (int) x + width, (int) y + height, spriteImage, 32, 0, 5));
		} else {
			game.flashDisplay
					.add(new Box((int) x, (int) y, (int) x + width, (int) y + height, spriteImage, 0, 0, 5));
		}
	}

	@Override
	public void update(Main game) {
		if (Math.abs(x - game.PC.x) < 16 && Math.abs(y - game.PC.y) < 16 && attack1 == 0) {
			game.PC.health = 32;
